...
int main()
{
    ...
    int r = 3;
    int spacing = 7;
    float h = 1.61803398875 * (float)spacing;
    float h2 = h * h;
    float cell_size = h;
    float predict_dt = DT;
    ...
    float gravity = -60.0f;
    float target_density = 0.023f;
    float pressure_multiplier = 55000.0f;
    float near_pressure_multiplier = 200.0f;
    float coefficient_of_viscosity = 12.5f;
    float velocity_damping = 1.0f;
    float collision_damping = 0.67f;
    float cohesion_multiplier = 1000.0f;

    float dist_threshold = 64;
    float attractor_multiplier = 5.0f;
    float repeller_multiplier = 5.0f;
    ...
    Vector2 *predicted_positions = malloc(N * sizeof(Vector2));
    ...
    char *prediction_shader_src = readFile("prediction_shader.comp");
    ...
    GLuint force_computation_program = createComputeShader(force_computation_src);
    GLuint update_program = createComputeShader(update_shader_src);
    GLuint predict_program = createComputeShader(prediction_shader_src);
    ...
    //predicted positions
    glGenBuffers(1, &predicted_positions_ssbo);
    glBindBuffer(GSSB, predicted_positions_ssbo);
    glBufferData(GSSB, N * sizeof(Vector2), predicted_positions, GL_DYNAMIC_COPY);
    glBindBufferBase(GSSB, 11, predicted_positions_ssbo);
    ...
    GLint force_h2_loc = glGetUniformLocation(force_computation_program, "h2");
    ...
    GLint force_cohesion_multiplier_loc = glGetUniformLocation(force_computation_program, "cohesion_multiplier");
    ...
    GLint force_dimensions_loc = glGetUniformLocation(force_computation_program, "dimensions");
    ...
    //prediction shader locations
    GLint predict_dt_loc = glGetUniformLocation(predict_program, "dt_predict");
    GLint predict_gravity_loc = glGetUniformLocation(predict_program, "gravity");
    GLint predict_num_particles_loc = glGetUniformLocation(predict_program, "num_particles");
    ...
    glUniform1f(force_cohesion_multiplier_loc, cohesion_multiplier);
    ...
    //send data to prediction shader
    glUseProgram(predict_program);

    glUniform1f(predict_dt_loc, predict_dt);
    glUniform1f(predict_gravity_loc, gravity);
    glUniform1i(predict_num_particles_loc, N);
    ...
    //glUniform1f(draw_target_density_loc, target_density);

    while (!glfwWindowShouldClose(window))
    {
        for (int iterations = 0; iterations < 8; iterations++)
        {
            //dispatch prediction shader
            glUseProgram(predict_program);
